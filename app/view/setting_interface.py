...
class SettingInterface(ScrollArea):
    ...
    def __initGroups(self):
        """初始化所有设置组"""
        # 视频下载配置组
        self.downloadGroup = SettingCardGroup(self.tr("视频下载"), self.scrollWidget)
        # 转录配置组
    ...
    def __initCards(self):
        ...
        self.helpCard = HyperlinkCard(
            ...
            FIF.HELP,
            self.tr("帮助"),
            self.tr("发现新功能并了解有关down-video-load的使用技巧"),
            self.aboutGroup,
        )
        self.feedbackCard = PrimaryPushSettingCard(
            ...
            FIF.FEEDBACK,
            self.tr("提供反馈"),
            self.tr("提供反馈帮助我们改进down-video-load"),
            self.aboutGroup,
        )
    ...
    def __createDownloadCards(self):
        """创建视频下载配置卡片"""
        # 分辨率选择
        self.downloadResolutionCard = ComboBoxSettingCard(
            cfg.download_resolution,
            FIF.VIDEO,
            self.tr("下载清晰度"),
            self.tr("选择视频下载的最高清晰度"),
            texts=[res.value for res in cfg.download_resolution.validator.options],  # type: ignore
            parent=self.downloadGroup,
        )

        # 字幕下载类型
        self.subtitleDownloadTypeCard = ComboBoxSettingCard(
            cfg.subtitle_download_type,
            FIF.FONT,
            self.tr("字幕策略"),
            self.tr("选择字幕下载策略"),
            texts=[t.value for t in cfg.subtitle_download_type.validator.options],  # type: ignore
            parent=self.downloadGroup,
        )

        # 代理检测
        self.proxyStrictCard = SwitchSettingCard(
            FIF.GLOBE,
            self.tr("强制代理检测"),
            self.tr("下载前强制检查网络代理（推荐开启，防止下载失败）"),
            cfg.proxy_strict_mode,
            self.downloadGroup,
        )

        # Cookie文件
        self.useCookieCard = SwitchSettingCard(
            FIF.ALBUM,
            self.tr("使用 Cookie"),
            self.tr("是否使用 cookies.txt 文件进行下载（非必要勿开启）"),
            cfg.use_cookie_file,
            self.downloadGroup,
        )

        self.downloadGroup.addSettingCard(self.downloadResolutionCard)
        self.downloadGroup.addSettingCard(self.subtitleDownloadTypeCard)
        self.downloadGroup.addSettingCard(self.proxyStrictCard)
        self.downloadGroup.addSettingCard(self.useCookieCard)
